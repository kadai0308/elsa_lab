...
class contentEdit extends Component {
    ...
    setSeason() {
        if (this.state.season) {
            document.getElementById(this.state.season).selected = 'selected'
        }
    }
    ...
    setUsers() {
        ...
        if (this.state.TAs) {
            this.state.TAs.map((item) => (
                ta_ids.push(item.id),
                ta_names.push(item.profile.name)
            ))
    ...
    handleSubmit(event){
        ...
        ins.put(`courses/${course_id}/contents/${content_id}`, this.state)
        .then((res) => {
            console.log(res)
                    ...
                    <div className="content-new-form">
                        <div className='content-new-input-group'>
                            <div className='input-group-right-40'>
                                ...
                                <input value={this.state.year} className='content-new-input' type="text" onChange={(e) => this.handleChange('year', e)}/>
                                ...
                                <input value={this.state.course_no} className='content-new-input' type="text" onChange={(e) => this.handleChange('course_no', e)}/>
                                <div className="input-label">time</div>
                                <input value={this.state.time} className='content-new-input' type="text" onChange={(e) => this.handleChange('time', e)}/>
